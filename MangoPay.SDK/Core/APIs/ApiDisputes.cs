...
namespace MangoPay.SDK.Core.APIs
{
    ...
    public class ApiDisputes : ApiBase
    {
		...
		public async Task CreateDisputePage(string disputeId, string documentId, string filePath)
		{
			byte[] fileArray = File.ReadAllBytes(filePath);
			await CreateDisputePage(disputeId, documentId, fileArray);
		}
		...
		public async Task CreateDisputePage(string disputeId, string documentId, byte[] binaryData)
		{
			await CreateDisputePage(null, disputeId, documentId, binaryData);
		}
		...
		/// <param name="documentId">Dispute document identifier.</param>
		/// <param name="binaryData">The byte array the DisputePage will be created from.</param>
		public async Task CreateDisputePage(string idempotencyKey, string disputeId, string documentId, byte[] binaryData)
		{
			string fileContent = Convert.ToBase64String(binaryData);

			DisputePagePostDTO disputePage = new DisputePagePostDTO(fileContent);

            await this.CreateObject<DisputePageDTO, DisputePagePostDTO>(idempotencyKey, MethodKey.DisputesDocumentPageCreate, disputePage, disputeId, documentId);
		}
